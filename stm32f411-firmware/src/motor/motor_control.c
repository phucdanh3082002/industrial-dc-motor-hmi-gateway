...
static void motor_state_enter(motor_state_t e_new_state, uint32_t u32_now_ms)
{
    ...
    switch (e_new_state) {
        ...
        case MOTOR_STATE_WARNING:
            u8_current_pwm = (u8_current_pwm * MOTOR_WARNING_PWM_REDUCE) / 100U;
            __HAL_TIM_SET_COMPARE(MOTOR_PWM_TIM, MOTOR_PWM_CHANNEL,
                                  (uint32_t)(u8_current_pwm * 99U / 100U));
            gpio_set_led(1U);
            gpio_set_buzzer(1U);
            uart_debug_printf("[MOTOR] State -> WARNING (PWM=%d%%)\r\n", u8_current_pwm);
            break;
...
int motor_control_init(void)
{
    ...
    uart_debug_printf("[MOTOR] Motor control initialized (PWM TIM1_CH1)\r\n");
    return 0;
}
...
int motor_control_update(const motor_command_t *ps_cmd)
{
    ...
    if (lm35_test_read_temp_c10(&s16_temp_c10) != 0) {
        ...
        uart_debug_printf("[MOTOR] LM35 read error\r\n");
    }

    if (ina219_test_read_bus_voltage_mv(&u16_voltage_mv) != 0) {
        u8_sensor_errors |= 0x02U;
        uart_debug_printf("[MOTOR] INA219 voltage read error\r\n");
    }

    if (ina219_test_read_current_ma(&s16_current_ma) != 0) {
        u8_sensor_errors |= 0x02U;
        uart_debug_printf("[MOTOR] INA219 current read error\r\n");
    }
    ...
    if (ps_cmd->u8_reset_fault == 1U && e_current_state == MOTOR_STATE_FAULT) {
        if (u8_sensor_errors == 0U) {
            ...
            motor_state_enter(MOTOR_STATE_STOP, u32_state_enter_ms);
        ...
        case MOTOR_STATE_STOP:
            if (ps_cmd->u8_enable == 1U && ps_cmd->u8_pwm_setpoint > 0U) {
                if (u8_sensor_errors == 0U) {
                    ...
                    uart_debug_printf("[MOTOR] Cannot start: sensor errors\r\n");
                }
            }
            break;
    ...
    if (e_next_state != e_current_state) {
        motor_state_exit(e_current_state);
        motor_state_enter(e_next_state, u32_state_enter_ms);
    }
    ...
    if (e_current_state == MOTOR_STATE_STOP || e_current_state == MOTOR_STATE_FAULT) {
        ...
        /* WARNING: PWM already reduced in state_enter, keep it */
        motor_apply_pwm(u8_current_pwm);
    } else if (e_current_state == MOTOR_STATE_RUNNING) {
        if (ps_cmd->e_mode == MOTOR_MODE_MANUAL) {
            motor_apply_pwm(ps_cmd->u8_pwm_setpoint);
        } else {
            /* AUTO mode: PWM from temperature lookup */
            uint8_t u8_auto_pwm = motor_auto_get_pwm(s16_temp_c10);
            motor_apply_pwm(u8_auto_pwm);
        }
    }

    /* --- 6. Debug output (every call) --- */
    uart_debug_printf("[MOTOR] S=%d PWM=%d%% T=%.1f°C I=%dmA V=%dmV ALM=0x%04X\r\n",
                      (int)e_current_state,
                      u8_current_pwm,
